...
public class SimEngine {
	...
	public SimResult runSimV2(Route toTraverse, GeoCoord startLoc, GeoCoord endLoc, ForecastReport report,
			...
			double minCharge, TreeMap<Integer, ForecastIO> inflectionPoints) throws NotEnoughChargeException {
	 ...
	 * 
	 * @param currForecast:
	 *            the FIODataPoint corresponding to the forecast at the
	 *            destination location at the current time for whichever
	 *            simFrame is calling this method
	 * @param toLoc:
	 *            the end location of the simFrame calling this method
	 * @param fromLoc:
	 *            the start location of the car in the simFrame calling this
	 *            method
	 * @param carSpeed:
	 *            the car's predicted speed in the simFrame calling this method
	 ...
	 * 
	 * @param ForecastIO
	 *            with full selection of weather data, and a set of hourly
	 *            forecasts that is sorted by time
	 * @param timeFrame:
	 *            time in ms since jan1 1970 (see System.currentTimeMillis)
	 ...
	 * 
	 * @param toLoc:
	 *            breadcrumb that simulated car is traveling to
	 * @param fromLoc:
	 *            breadcrumb that simulated car is traveling from
	 * @param carSpeed:
	 *            the simulated car's current speed (from the previous
	 *            breadcrumb) in km/h
	 * @param toForecast:
	 *            the weather forecast at the current time at the destination
	...
	/*
	 * Did RequestedSpeeds as a Map<CeoCoord point, Map<lap number, requested
	 * speed>>, to be able to request different speeds for different laps.
	 * 
	 * public List<SimFrame> runSimulation(Route toTraverse, int
	 * startLocationIndex, ForecastReport weatherReports, TelemDataPacket
	 * carStartingCondition, Map<GeoCoord,Map<Integer,Double>> requestedSpeeds,
	 * int laps){ if(laps <= 0){ throw new IllegalArgumentException(
	 * "Must go at least one lap"); }
	 * 
	 * SolarLog.write(LogType.SYSTEM_REPORT, System.currentTimeMillis(),
	 * "simulation starting"); List<SimFrame> listOfFrames = new
	 * ArrayList<SimFrame>(toTraverse.getTrailMarkers().size());
	 * if(startLocationIndex == toTraverse.getTrailMarkers().size()-1){ return
	 * new ArrayList<SimFrame>(); //can't simulate if at end of race. }
	 * 
	 * ForecastIO weather =
	 * weatherReports.getForecasts().get(startLocationIndex); //assumes that the
	 * number of forecasts in weatherReports = number in Route. GeoCoord start =
	 * toTraverse.getTrailMarkers().get(startLocationIndex); GeoCoord next =
	 * toTraverse.getTrailMarkers().get(startLocationIndex + 1); //won't index
	 * out of range because of check above. if(requestedSpeeds.get(start) !=
	 * null && requestedSpeeds.get(start).get(1) != null){ Double reqSpeed =
	 * requestedSpeeds.get(start).get(1);//first lap } TelemDataPacket
	 * startCondition = carStartingCondition; FIODataPoint startWeather = new
	 * FIODataBlock(weather.getHourly()).datapoint(0); LocationReport
	 * simmedStartPoint = new
	 * LocationReport(toTraverse.getTrailMarkers().get(startLocationIndex),
	 * "Raven", "Simmed"); SimFrame startFrame = new SimFrame(startWeather,
	 * startCondition, simmedStartPoint, System.currentTimeMillis(), 1);
	 * //starting frame is current. listOfFrames.add(startFrame);
	 * 
	 * SimFrame lastFrame = startFrame; int numOfPoints =
	 * toTraverse.getTrailMarkers().size(); int currentLap = 1; for(int i =
	 * startLocationIndex+1; i<(numOfPoints*laps); i++){
	 *
	 * 
	 * By starting at startPos, we calculate the jump from car's current
	 * location to the next breadcrumb, rather than just assuming that it's
	 * actually at the last breadcrumb. This way may produce errors if the car
	 * is actually far off the trail, but the alternative is to advance the car
	 * magically to next breadcrumb, and if the gap between breadcrumbs is big,
	 * it may produce an error.
	 *
	 * 
	 * ForecastIO nextWeather; GeoCoord nextPoint; if
	 * (lastFrame.getCarStatus().getSpeed()<=0){ i--; //if the speed is zero
	 * then we need to redo the frame because the car is not moving, and thus is
	 * in the same place if(i%numOfPoints == 0){ currentLap--;//was adjusted
	 * earlier above, shouldn't have been. } if(i<0){ nextWeather =
	 * weatherReports.getForecasts().get(startLocationIndex); } else{
	 * nextWeather = weatherReports.getForecasts().get(i%numOfPoints); }
	 * 
	 * nextPoint = lastFrame.getGPSReport().getLocation(); } else{ nextWeather =
	 * weatherReports.getForecasts().get(i%numOfPoints); nextPoint =
	 * toTraverse.getTrailMarkers().get(i%numOfPoints); } Double
	 * requestedSpeedTemp = null; if(requestedSpeeds.get(nextPoint) != null){
	 * requestedSpeedTemp = requestedSpeeds.get(nextPoint).get(currentLap); }
	 * SimFrame nextFrame = this.generateNextFrame(lastFrame, nextPoint,
	 * nextWeather, requestedSpeedTemp,currentLap);
	 * 
	 * lastFrame = nextFrame; listOfFrames.add(nextFrame); if(i%numOfPoints ==
	 * (numOfPoints-1)){ //if it's the last point in the lap currentLap++; //say
	 * we're going to the next lap! } }
	 * 
	 * return listOfFrames; }
	 * 
	 * 
	 * 
	 * private SimFrame generateNextFrame(SimFrame lastFrame, GeoCoord
	 * nextPoint, ForecastIO nextWeather, Double requestedSpeed,int currentLap)
	 * { TelemDataPacket lastCarStatus = lastFrame.getCarStatus(); GeoCoord
	 * lastPosition = lastFrame.getGPSReport().getLocation(); double lastSpeed =
	 * lastCarStatus.getSpeed(); long lastTimeStamp =
	 * lastFrame.getRepresentedTime();
	 * 
	 * double elevationChange = nextPoint.getElevation() -
	 * lastPosition.getElevation();
	 * 
	 * double speedToDrive; if(requestedSpeed == null){ speedToDrive =
	 * calculateBestSpeed(lastCarStatus.getSpeed(), elevationChange,
	 * lastCarStatus.getStateOfCharge()); //stubMethod. Also this is a greedy
	 * algo. //obviously need to add more arguments ^^ }
	 * //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^KEEP ALL
	 * ABOVE^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 * else{ speedToDrive = requestedSpeed; } //
	 ...
	 * 
	 * //not sure if calculateDistance() takes elevation into account.... double
	 * distanceCovered = lastPosition.calculateDistance(nextPoint)*1000; long
	 * nextSimFrameTime; long timeSinceLastFrame; double timeSinceLastFrameInHr;
	 * 
	 * if (speedToDrive >= .001){ double tempTime =
	 * (distanceCovered/(speedToDrive * 1000.0))*60.0*1000.0*60.0; //double
	 * check units. km/h and m?? distanceCovered is in meters timeSinceLastFrame
	 * = (long) tempTime; nextSimFrameTime = lastTimeStamp + timeSinceLastFrame;
	 * timeSinceLastFrameInHr = timeSinceLastFrame/(60.0*1000.0*60.0);
	 * //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ENCLOSED IS GOOD
	 * STUFF^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 * ^^^^^ // STUFF BELOW HERE TO THE NEXT LINE I AM NOT TOO FAMILIAR WITH
	 * //System.out.println(distanceCovered); } else{ double tempTime =
	 * RECHARGE_TIME_MS; //double check units. km/h and m?? distanceCovered is
	 * in meters timeSinceLastFrame = (long) tempTime; nextSimFrameTime =
	 * lastTimeStamp + timeSinceLastFrame; timeSinceLastFrameInHr =
	 * tempTime/(1000*60*60); //System.out.println("AGAHAHAHAHAHAHAH RAN");
	 * //System.out.println(timeSinceLastFrameInHr);
	 * //System.out.println(distanceCovered); }
	 * 
	 * 
	 * 
	 * FIODataPoint forecastForPoint = chooseReport(nextWeather,
	 * nextSimFrameTime); double sunPowerInWatts =
	 * calculateSunPower(forecastForPoint);
	 * 
	 * 
	 * double SunCharge =
	 * (sunPowerInWatts*timeSinceLastFrameInHr)/(inUseCarModel.getMaxBatteryCap(
	 * )); //divide watt hrs from the sun by max watt hrs to get the percentage
	 * of charge from the sun if(SunCharge>2000000){
	 * SolarLog.write(LogType.SYSTEM_REPORT, System.currentTimeMillis(), "" +
	 * distanceCovered + " " + timeSinceLastFrameInHr + " Speed: " +
	 * speedToDrive); }
	 * 
	 * TelemDataPacket newCarStatus; newCarStatus =
	 * calculateNewCarStatus(lastCarStatus, distanceCovered, elevationChange,
	 * forecastForPoint, speedToDrive, SunCharge); LocationReport
	 * nextLocationReport = generateLocationReport(lastFrame.getGPSReport(),
	 * nextPoint);
	 * 
	 * SimFrame toReturn = new SimFrame(forecastForPoint, newCarStatus,
	 * nextLocationReport, nextSimFrameTime,currentLap);
	 * 
	 * try { Date fcParseTime =
	 * GlobalValues.forecastIODateParser.parse(toReturn.getForecast().time()); }
	 ...
	 * return toReturn; }
	 * 
	 * private LocationReport generateLocationReport(LocationReport oldReport,
	 * GeoCoord nextPoint) { LocationReport toReturn = new
	 * LocationReport(nextPoint, oldReport.getCarName(), "Simulated"); return
	 * toReturn; }
	 */

	/*
	 * //timeTaken is in hours private TelemDataPacket
	 * calculateNewTelemPacket(TelemDataPacket prevStatus, GeoCoord startLoc,
	 * GeoCoord endLoc, FIODataPoint forecastForPoint, double speed, double
	 * timeTaken){ double changeInChargePerCent =
	 * calculateChargeDiff(startLoc,endLoc,forecastForPoint,speed,timeTaken);
	 * double newCharge = prevStatus.getStateOfCharge()+changeInChargePerCent;
	 * if(newCharge > 100) newCharge = 100; if(newCharge < 0) newCharge = 0;
	 * 
	 * TelemDataPacket newStatus = new TelemDataPacket(speed,
	 * prevStatus.getTotalVoltage(), prevStatus.getTemperatures(),
	 * prevStatus.getCellVoltages(), newCharge);
	 * 
	 * return newStatus;
	 * 
	 * }
