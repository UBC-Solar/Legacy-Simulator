...
public class GUImain implements Listener {
	...
	private GlobalController mySession; // Global Controller for the program
										// (interface between code and UI)
	private JPanel carPanel; // Car status within the main window
	private CustomDisplayMap mainPanel; // Biggest panel in the main window;
										// Shows amalgamated information
	...
	private JPanel loadStatusPanel; // Shows the loaded status of modules at a
									// quick glance
	...
	/**
	 * creates all windows that can be launched from this main panel. Does not
	 * overwrite any that currently exist! Should close those before calling
	 * this.
	 */
	private void buildAllWindows() {
		if (this.simFrame == null) {
			this.simFrame = new SimulationAdvancedWindow(this.mySession, this); // Sim
																				// advanced
																				// window
		}
		if (this.carFrame == null) {
			this.carFrame = new CarAdvancedWindow(this.mySession); // Car
																	// advanced
																	// window
		}
		if (this.mapFrame == null) {
			this.mapFrame = new MapAdvancedWindow(this.mySession); // Map
																	// advanced
																	// window
		}
		if (this.weatherFrame == null) {
			this.weatherFrame = new WeatherAdvancedWindow(this.mySession, this); // Weather
																					// advanced
																					// window
		}
	...
	/**
	 * registers for all notification classes that this window needs to listen
	 * for.
	 */
	@Override
	...
	private void drawNewMap(NewMapLoadedNotification notification) {
		/*
		 * Coordinate vancouver = new Coordinate(49.282,-123.12); Coordinate
		 * merritt = new Coordinate(50.1119,-120.78); Coordinate thirdPoint =
		 * new Coordinate(49.282, -120.78); mainPanel.addMapMarker(new
		 * MapMarkerDot(vancouver)); //Vancouver mainPanel.addMapMarker(new
		 * MapMarkerDot(merritt)); //merritt mainPanel.addMapMarker(new
		 * MapMarkerDot(thirdPoint)); //third point to make a triangle
		 * List<Coordinate> toAdd = new ArrayList<Coordinate>(3);
		 * toAdd.add(vancouver); toAdd.add(merritt); toAdd.add(thirdPoint);
		 * mainPanel.addMapPolygon(new MapPolygonImpl(toAdd));
		 * System.out.println(n.getRoute().getTrailMarkers().get(0));
		 */
		/*
		 * Route temp = notification.getRoute(); List<Coordinate> listForPolygon
		 * = new ArrayList<Coordinate>(temp.getTrailMarkers().size());
		 * for(GeoCoord geo : temp.getTrailMarkers()){ listForPolygon.add(new
		 * Coordinate(geo.getLat(), geo.getLon())); }
		 * 
		 * //adding this in to make it a single line, otherwise it draws a line
		 * from end to start. for(int i = temp.getTrailMarkers().size()-1; i>=0;
		 * i--){ GeoCoord toAdd = temp.getTrailMarkers().get(i);
	...
	/**
	 * This method basically creates the entire UI. Creates a new mainframe,
	 * then generates and adds all sub components.
	 * 
	 * @param parent
	 */
	private void initialize(GlobalController parent) {
		...
		double programWindowHeight = screenHeight / 1.05; // leave a little gap
															// at top
		double programWindowStartx = (screenWidth - programWindowWidth) / 2; // center
																				// it
		double programWindowStarty = 0 + (screenHeight / 40); // give a little
																// buffer.

		// public void setBounds(int x, int y, int width, int height)
		mainFrame.setBounds((int) programWindowStartx, (int) programWindowStarty, (int) programWindowWidth,
				(int) programWindowHeight); // main window size on opening
		mainFrame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
		mainFrame.setVisible(true);
		// NOTE: Could consider not building all the windows at once in case of
		// performance issues.
		this.buildAllWindows(); // builds all the windows in one shot
		JMenuBar menuBar = new JMenuBar();
		mainFrame.setJMenuBar(menuBar);

		// adding this to gracefully close the program when the 'x' is hit on
		// the main frame.
		...
		mainFrame.getContentPane()
				.setLayout(new FormLayout(
						new ColumnSpec[] { ColumnSpec.decode("300px"), FormFactory.RELATED_GAP_COLSPEC,
								ColumnSpec.decode("default:grow"), },
						new RowSpec[] { RowSpec.decode("0px"), // this is the
																// row for the
																// status panel
		...
		mainFrame.repaint(); // sometimes the map window button doesn't pop up,
								// hoping this fixes the glitch
	}
	...
	public void launchWeather() {
		...
		// if(weatherFrame.getState()!=Frame.NORMAL) {
	...
	public void tryCloseProgram() {
		if (JOptionPane.showConfirmDialog(mainFrame, "Are you sure you want to quit?", "Quit Program?",
